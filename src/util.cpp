...
const char * const BITHAO_CONF_FILENAME = "bithao.conf";
const char * const BITHAO_PID_FILENAME = "bithaod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bithao" is a composite category enabling all BitHao-related debug output
            if(ptrCategory->count(string("bithao"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bithaoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bithaoCore
    // Mac: ~/Library/Application Support/bithaoCore
    // Unix: ~/.bithaoCore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bithaoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bithaoCore";
#else
    // Unix
    return pathRet / ".bithaoCore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", BITHAO_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bithao.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bithao.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", BITHAO_PID_FILENAME));
